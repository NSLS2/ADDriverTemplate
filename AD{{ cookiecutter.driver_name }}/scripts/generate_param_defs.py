...
def generate_header_file_for_db(params, output_dir, base_name):
    ...
    with open(header_file, "w") as hf:
        ...
        for param in params:
            logger.info(f"Defining param string: {param.name} as {param.record_str}")
            hf.write(f'#define {param.name}String "{param.record_str}"\n')
        hf.write("\n")

        hf.write("// Parameter index definitions\n")
        for param in params:
            logger.info(f"Defining param index for param: {param.name}")
...
def main():
    ...
    parser.add_argument("input_dir", help="Directory containing the DB template files.")
    parser.add_argument(
    ...
    for file in os.listdir(input_dir):
        template_file_path = os.path.join(input_dir, file)
        if file.endswith(".template"):
            base_name = file.rsplit(".", 1)[0]
            database = dbtoolspy.load_database_file(template_file_path, macros=args.macros)
            params = get_params_from_db(database, base_name)
            for param in params:
                logger.info(f"Found parameter: {param.name} of type {param.type}")
